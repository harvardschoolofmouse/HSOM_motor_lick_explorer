...
%% 
        ...
        function [ax,f] = plotMeanPixels(obj,Style,ax, Limits)
		...
		function [ax,f] = plotTrialStarts(obj)
			%
			%	must run alignCED_to_video and getVideoTrialStartFrames first
			%
			threshold = obj.ROIs.lampOFF.threshold;
			[ax,f] = obj.plotMeanPixels('lampOFF');
			xx = get(ax, 'xlim');
		...
		function UIcleanUpTrialStarts(obj)
        	...
        	for ii = 1:numel(trial_starts_in_question)
            	...
            	title(ax,num2str(trial_starts_in_question(ii)))
                % use the CED timestamp?
            	answer = questdlg('Shift video to match CED (y) or use CED timestamp (n)?');
            	if strcmp(answer, 'Yes')
                    % add nans to the ROI mean pixels fields
                    CEDstart = frames_starts_in_question(ii);
                    xx= find(obj.video.lampOFF.frames <= CEDstart, 1, 'last');
                    videostart = obj.video.lampOFF.frames(xx);
                    frameshift = CEDstart - videostart;
                    obj.video.lampOFF.mean_pixels = [obj.video.lampOFF.mean_pixels(1:videostart);nan(frameshift,1);obj.video.lampOFF.mean_pixels(videostart+1:end)];
                    obj.video.lick.mean_pixels = [obj.video.lick.mean_pixels(1:videostart);nan(frameshift,1);obj.video.lick.mean_pixels(videostart+1:end)];
                    obj.video.lampOFF.frames(xx+1:end) = obj.video.lampOFF.frames(xx+1:end) + frameshift;
                    obj.video.lampOFF.frames(xx) = []; 
		...
		function getVideoEventsInCEDsec(obj)
            ...
            if numel(obj.video.frames_s) < numel(obj.video.lick.mean_pixels)
                warning('this is a remnant of adding nans due to lost frames? may want to revise')
                obj.video.frames_s = [obj.CED.CamO_s_trim;nan(numel(obj.video.lick.mean_pixels) - numel(obj.CED.CamO_s_trim),1)];
                % nanidx = find(isnan(obj.video.lick.mean_pixels));
                % while numel(nanidx) > 0
                %     % get the contiguous idx
                %     diff = nanidx(2:end) - nanidx(1:end-1);
                %     endcontigix = find(diff ~= 1, 1, 'first');
                %     startnan = nanidx(1);
                %     endnan = nanidx(endcontigix);
                %     obj.video.frames_s = [obj.video.frames_s(1:startnan-1);nan(endnan-startnan + 1,1);obj.video.frames_s(startnan:end)];
                %     nanidx(1:endcontigix) = [];
                % end
            end
